...
15a) When the observed text explicitly names a specific leavening product, such as dessert baking powder/cake leavener, and the matching target exists in context, prefer approve_alias to that target over needs_human_review. If the target is absent but the identity is clear, prefer create_canonical draft over vague review.
...
17) Read global_learning_memory, each work item's relevant_learning_memory, each work item's training_signals, and each work item's external_catalog_evidence before deciding.
...
17c) Treat external_catalog_evidence as grounding evidence only. It can support ingredient existence, synonym, taxonomy, variant, branded-product, or nutrition reasoning, but it is not Season catalog truth and must not bypass catalog_matcher, learning memory, deterministic validators, or apply gates.
17d) Cite external source_key/evidence_type in evidence when external_catalog_evidence materially changes confidence or proposal type. If external evidence is needs_review, use it as weak support or to ask better blocking questions.
...
33b) Broad aggregate/category terms such as generic spices, herbs, seasonings, aromas, vegetables, fruit, seafood, or cheese should not become a new canonical ingredient from the category word alone. Use create_canonical only when recipe context names a recognized concrete product/blend/mix or an identity-bearing aggregate; otherwise return needs_human_review with the missing specificity question.
33c) Recipe-process byproducts such as cooking water, pasta water, soaking liquid, or generic cooking liquid are usually technique/context, not standalone fridge/catalog ingredients. Do not create a canonical for them unless the text identifies a reusable ingredient such as broth, stock, aquafaba, or another named liquid.
...
41) Apply the creator-facing generic base rule generally, not only to named examples: simple concrete base ingredients such as potatoes, tomatoes, mushrooms, apples, onions, or olives are acceptable base identities in fast/easy recipes. If a safe base target exists, prefer approve_alias to that base. If no safe base exists but child/specific variants exist, prefer create_canonical draft for the missing base. Do not force review only because a structured recipe could have used a more specific variant.
42) Keep the counterweight: broad umbrella categories such as vegetables, fruit, herbs, spices, cheeses, seafood, fish, or seasonings are not concrete base ingredients by themselves. Specific structured forms, cultivars, cuts, product states, dietary variants, protected designations, and preparation-sensitive variants should remain child/specific targets or reviewed catalog gaps.
`;
