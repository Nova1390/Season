-- Recreate live candidate creation RPC to remove remaining ingredient_id ambiguity paths.
...
begin
  ...
  if p_create_alias and v_alias_normalized is not null and v_alias_normalized <> '' then
    ...
    if v_active_alias_id is not null and v_active_alias_ingredient_id is distinct from v_created_ingredient_id then
      raise exception
    ...
    if v_active_alias_id is null then
      ...
      set
        ...
        language_code = coalesce(v_language_code, a.language_code),
        source = 'manual',
