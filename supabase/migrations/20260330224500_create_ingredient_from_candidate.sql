-- Minimal backend-only workflow to create a canonical ingredient from a reviewed candidate.
-- No reconciliation or recipe write-back is performed here.

-- Extend decision action enum-like check to include canonical ingredient creation actions.
alter table public.catalog_candidate_decisions
  drop constraint if exists catalog_candidate_decisions_action_check;

alter table public.catalog_candidate_decisions
  add constraint catalog_candidate_decisions_action_check
  check (
    action in (
      'approve_alias',
      'reject_alias',
      ...
      'ignore',
      'create_ingredient_from_candidate',
      'create_ingredient_from_candidate_existing'
    )
  );
...
begin
  ...
  -- fail if an existing canonical ingredient is already strongly linked to this concept.
  select i.id
