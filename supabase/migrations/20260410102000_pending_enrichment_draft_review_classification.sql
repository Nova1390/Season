...
begin
  ...
  features as (
    select
      ...
      (
        p.normalized_text ~* '\\b\\d+(?:[.,]\\d+)?\\s*(g|kg|gr|ml|l|cl|pizzico|pizzichi|cucchiaio|cucchiai|cucchiaino|cucchiaini|tbsp|tsp|cup)\\b'
        or p.normalized_text ~* '\\s\\d+$'
      )
      and p.normalized_text <> 'farina 00' as quantity_contaminated,
      p.normalized_text ~* '(ammorbidito|da grattugiare|in grani|a temperatura ambiente|freddo di frigo|fresco|fresca|tritato|tritata)' as low_risk_qualifier,
