...
begin
  ...
  if v_result_ingredient_id is not null then
    ...
    from (
      values
        ('it', nullif(trim(v_draft.canonical_name_it), '')),
        ('en', nullif(trim(v_draft.canonical_name_en), ''))
    ) as localization(language_code, display_name)
    where localization.display_name is not null
...
select
  ...
  backfill.language_code,
  backfill.display_name,
  null,
  now()
from (
  select distinct on (i.id, localization.language_code)
  ...
  cross join lateral (
    values
      ('it', nullif(trim(d.canonical_name_it), '')),
      ('en', nullif(trim(d.canonical_name_en), ''))
  ) as localization(language_code, display_name)
  where d.status = 'applied'
    and localization.display_name is not null
  order by i.id, localization.language_code, d.updated_at desc
) backfill
on conflict on constraint ingredient_localizations_pkey do update
set
  display_name = excluded.display_name,
...
comment on function public.create_catalog_ingredient_from_enrichment_draft(text, text, double precision) is
  'Creates a catalog ingredient from a ready enrichment draft and preserves all available draft localizations.';
