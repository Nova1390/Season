-- Read-only policy audit for the remaining Giallo Zafferano replay gaps.
--
-- At this point the resolver has zero exact-match fallbacks. The remaining
-- differences are cases where recipe-specific qualifiers were modeled as
-- catalog variants during cleanup, while today's resolver maps to the base
-- catalog item. This view separates preparation/usage notes from possible
-- true catalog variants before we mutate recipe JSON or catalog aliases.
...
select
  ...
  case
    when ingredient_name ~* '\ba\s+temperatura\s+ambiente\b'
      or ingredient_name ~* '\bammorbidit[oaie]\b'
      then 'preparation_state'
    when ingredient_name ~* '\bda\s+grattugiare\b'
      then 'usage_instruction'
    when ingredient_name ~* '\ba\s+pasta\s+gialla\b'
      then 'catalog_variant_candidate'
    when ingredient_name ~* '\bdi\s+uova\s+medie\b'
  ...
  case
    ...
    when ingredient_name ~* '\bammorbidit[oaie]\b'
      then 'ammorbidito'
    when ingredient_name ~* '\bda\s+grattugiare\b'
      then 'da grattugiare'
    when ingredient_name ~* '\ba\s+pasta\s+gialla\b'
  ...
  case
    when ingredient_name ~* '\ba\s+temperatura\s+ambiente\b'
      or ingredient_name ~* '\bammorbidit[oaie]\b'
      or ingredient_name ~* '\bda\s+grattugiare\b'
      or ingredient_name ~* '\bdi\s+uova\s+medie\b'
