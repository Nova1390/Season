-- Refine admin hierarchy advisory hints from single-action to multi-option guidance.
-- Advisory only: no runtime matching/reconciliation behavior changes.
...
begin
  ...
  from (
    ...
    select
      ...
      s.priority_score,
      (s.strong_parent_slug is not null or s.weak_parent_slug is not null) as canonical_parent_exists,
      ...
      case
        when coalesce(s.match_count, 0) = 1 then 'Exact canonical match found; alias is the most likely review outcome.'
        when s.strong_parent_slug is not null and s.close_child_exists then 'Strong parent context with nearby existing child variants; review alias vs new variant.'
  ...
  return jsonb_build_object(
    ...
    'metadata', jsonb_build_object(
      ...
      'source', 'catalog_admin_ops_snapshot_v5_multi_option_hints'
    )
  );
end;
$$;
