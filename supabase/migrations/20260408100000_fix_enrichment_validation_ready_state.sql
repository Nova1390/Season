-- Fix validation-ready state: ready should depend on validation errors only.
-- If validation_errors is empty, validated_ready must be true.
...
begin
  ...
  -- Validation state should mirror error presence only.
  v_is_ready := coalesce(array_length(v_errors, 1), 0) = 0;

  update public.catalog_ingredient_enrichment_drafts as d
  set
    validated_ready = v_is_ready,
