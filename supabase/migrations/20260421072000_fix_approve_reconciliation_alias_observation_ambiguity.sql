-- Fix approve_reconciliation_alias after alias-writer unification.
-- The observation update must qualify normalized_text because the function returns
-- a column with the same name, which makes unqualified references ambiguous.
