-- Add per-item failure observability for batch triage execution.
-- Keeps existing business logic, adds explicit error_message output and server notices.
...
begin
  ...
  loop
    ...
    begin
      ...
      elsif v_action = 'prepare_enrichment_draft' then
        select
          d.status,
          d.validated_ready
        into
    ...
    exception
      when others then
        v_result_status := 'failed';
        v_error_message := coalesce(sqlerrm, 'unknown_error');
        v_detail := v_error_message;
        v_failed := v_failed + 1;
        raise notice 'execute_catalog_candidate_batch_triage failed: action=% normalized_text=% error=%',
          coalesce(nullif(v_action, ''), nullif(v_action_raw, ''), 'unknown'),
          coalesce(nullif(v_normalized, ''), '<empty>'),
          v_error_message;
    end;
    ...
    v_results := v_results || jsonb_build_array(
      jsonb_build_object(
        ...
        'error_message', case when v_result_status = 'failed' then v_error_message else null end,
        'detail', v_detail,
  ...
  return jsonb_build_object(
    ...
    'metadata', jsonb_build_object(
      'processed_at', v_now,
      'processed_by', v_user,
      'source', 'catalog_candidate_batch_triage_v2_observable'
    )
  );
end;
$$;
