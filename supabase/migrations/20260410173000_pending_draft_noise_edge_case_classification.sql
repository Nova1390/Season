-- Tighten pending draft review classifier for noisy edge-case text.
-- Scope: read-only review buckets only; no enrichment/create pipeline changes.
...
begin
  ...
  alias_stats as (
    select
      p.normalized_text,
      bool_or(a.id is not null and coalesce(a.is_active, true) and coalesce(a.status, 'approved') = 'approved') as has_approved_alias,
      bool_or(a.id is not null) as has_any_alias_match
    from pending p
  ...
  features as (
    select
      ...
      (
        -- HTML-entity noise (e.g. &frac12) should never go to enrichment.
        p.normalized_text ~* '&(?:[a-z]{2,12}|#\\d{2,6}|#x[0-9a-f]{2,6});?'
        or (
          p.normalized_text <> 'farina 00'
          and (
            -- Measured quantities and trailing numeric counts.
            p.normalized_text ~* '\\b\\d+(?:\\/\\d+|(?:[.,]\\d+)?)\\s*(?:g|kg|gr|mg|ml|l|cl|pizzico|pizzichi|mazzetto|mazzetti|ciuffo|ciuffi|cucchiaio|cucchiai|cucchiaino|cucchiaini|spicchio|spicchi|foglia|foglie|cup|tbsp|tsp)\\.?\\b'
            or p.normalized_text ~* '\\b(?:g|kg|gr|mg|ml|l|cl)\\s*\\d+(?:[.,]\\d+)?\\b'
            or p.normalized_text ~* '\\s\\d+(?:[.,]\\d+)?$'
            -- Mixed descriptors with quantity tails (e.g. "cipolle rosse 400 g", "uova medie 3").
            or p.normalized_text ~* '\\b(?:fino|grosso|rosse|rossa|rossi|rosso|medie|media|medio|intero|intera|fresco|fresca|extravergine|evo|da grattugiare|in grani)\\b.*\\b\\d+(?:[.,]\\d+)?(?:\\s*(?:g|kg|gr|mg|ml|l|cl|pizzico|pizzichi|mazzetto|mazzetti|ciuffo|ciuffi))?\\b'
          )
        )
