-- Backend-driven safe auto-alias approval for high-confidence alias-like candidates.
-- Reduces manual operator toil while preserving strict safety and auditability.

create table if not exists public.catalog_alias_auto_apply_audit (
...
create index if not exists idx_catalog_alias_auto_apply_audit_created_at
  on public.catalog_alias_auto_apply_audit(created_at desc);

create index if not exists idx_catalog_alias_auto_apply_audit_result_status
  on public.catalog_alias_auto_apply_audit(result_status);

alter table public.catalog_alias_auto_apply_audit enable row level security;

revoke all on public.catalog_alias_auto_apply_audit from public, anon, authenticated;
grant all on public.catalog_alias_auto_apply_audit to service_role;
...
begin
  ...
  for r in
    ...
    exact_match as (
      select
        p.normalized_text,
        count(distinct ck.ingredient_id)::integer as target_count,
        min(ck.ingredient_id) as ingredient_id,
        min(ck.ingredient_slug) as ingredient_slug
      from prepared p
