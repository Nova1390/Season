...
- 2026-05-15 inbox hygiene automation: `cleanup_catalog_agent_review_inbox(...)` now supersedes older duplicate open proposals per normalized term and `run-catalog-agent-triage` calls it at run start. The cleanup preserves audit history and keeps the admin console focused on the single best open item per ingredient.
