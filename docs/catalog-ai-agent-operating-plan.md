...
- Current autonomy maturity is `7.0 dev autonomy foundation`: the agent can run bounded multi-pass LLM reasoning, persist governed proposals when explicitly enabled, delegate dry-run workers, survive scheduled dev shift checks, and self-repair blocked proposal output without mutating the catalog directly. The staged path to `8.0+` is defined in `docs/catalog-agent-autonomy-roadmap.md`.
