...
Status: planning document plus implementation roadmap. The dev runtime now covers proposal generation, governed low-risk worker delegation, dry-run scheduling, and Level `7.0` quality-gate self-repair. Staging autonomy is still out of scope.
...
- Proposal persistence is fail-closed behind `CATALOG_AGENT_PROPOSAL_PERSISTENCE_ENABLED` and every proposal must pass a runtime quality gate before insert.
- The first context-enriched agent snapshot is documented in `docs/catalog-agent-context-enrichment.md`.
...
- Current autonomy maturity is `7.0 dev autonomy foundation` with the first `7.1` building blocks: the agent can run bounded multi-pass LLM reasoning, persist governed proposals when explicitly enabled, delegate dry-run workers, survive scheduled dev shift checks, self-repair blocked proposal output, attach deterministic catalog-matcher hints, and emit learning-writer suggestions without mutating the catalog directly. The staged path to `8.0+` is defined in `docs/catalog-agent-autonomy-roadmap.md`.
...
- Level `7.0` is not full autonomous creation: canonical ingredient creation still requires enrichment drafts, validators, governed workers, audit, and explicit dev-only enablement.
