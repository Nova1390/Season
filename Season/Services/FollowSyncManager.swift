...
final class FollowSyncManager {
    ...
    func syncFromBackend() async {
        ...
        SeasonLog.debug("[SEASON_SUPABASE] request=syncFollowFromBackend phase=request_ok follower_id=\(followerID) fetched=\(remoteRelations.count) merged=\(addedCount)")
    ...
    func syncToBackend() async {
        ...
        for relation in pendingRelations {
            guard areValidUUIDs(followerId: relation.followerId, followingId: relation.followingId) else {
                SeasonLog.debug("[SEASON_FOLLOW_IDENTITY] phase=backend_sync_skipped_invalid_uuid follower_id=\(normalized(relation.followerId)) following_id=\(normalized(relation.followingId))")
            ...
            case .create:
                SeasonLog.debug("[SEASON_FOLLOW_SYNC] phase=backend_sync_allowed operation=create follower_id=\(normalized(relation.followerId)) following_id=\(normalized(relation.followingId))")
                didSync = await supabaseService.createFollow(relation)
            case .delete:
                SeasonLog.debug("[SEASON_FOLLOW_SYNC] phase=backend_sync_allowed operation=delete follower_id=\(normalized(relation.followerId)) following_id=\(normalized(relation.followingId))")
        ...
        let followerID = normalized(CurrentUser.shared.creator.id)
        SeasonLog.debug("[SEASON_SUPABASE] request=syncFollowToBackend phase=request_ok follower_id=\(followerID) pending=\(pendingRelations.count) processed=\(processedCount)")
    }
    ...
    private func shouldSyncToBackend(followerId: String, followingId: String) -> Bool {
        ...
        guard let authenticatedFollowerID = supabaseService.currentAuthenticatedUserID()?.uuidString.lowercased(),
            ...
            SeasonLog.debug("[SEASON_FOLLOW_SYNC] phase=backend_sync_skipped_local_creator follower_id=\(normalizedFollower) following_id=\(normalizedFollowing)")
            return false
        }
