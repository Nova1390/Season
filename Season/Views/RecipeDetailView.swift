...
struct RecipeDetailView: View {
                    ...
                    ScrollView {
                        VStack(spacing: 0) {
                            ...
                            VStack(spacing: 0) {
                                VStack(spacing: DS.Spacing.xl) {
                                    ...
                                    CreatorBarView(
                                        ...
                                        onCreatorTap: {
                                            ...
                                            SeasonLog.debug("[SEASON_FOLLOW_IDENTITY] phase=push_profile recipe_id=\(rankedRecipe.recipe.id) creator_id=\(validRecipeCreatorID ?? "nil") creator_name=\(displayedCreatorName)")
                                        ...
                                        onFollowTap: {
                                            if let creatorID = validRecipeCreatorID {
                                                ...
                                                SeasonLog.debug("[SEASON_FOLLOW_RECIPE] phase=top_icon_tap recipe_id=\(rankedRecipe.recipe.id) creator_id=\(creatorID) state_before=\(stateBefore) state_after=\(stateAfter)")
                                            }
                                        }
                                    )
                                    ...
                                    if !confirmedDietaryTags.isEmpty {
                                        dietaryTagsSection(tags: confirmedDietaryTags)
                                    }
                                ...
                                .padding(.bottom, SeasonLayout.bottomBarContentClearance + DS.Spacing.xxl)
                            }
                            .frame(width: geometry.size.width)
                            .background(
                ...
                HStack(spacing: 10) {
                    Button {
                        ...
                        Image(systemName: viewModel.isRecipeSaved(rankedRecipe.recipe) ? "bookmark.fill" : "bookmark")
                            ...
                            .foregroundColor(viewModel.isRecipeSaved(rankedRecipe.recipe) ? DS.Color.sageDeep : DS.Color.inkSoft)
    ...
    private var titleAndMetadataBlock: some View {
        VStack(alignment: .leading, spacing: DS.Spacing.sm) {
            HStack(spacing: 7) {
                Circle()
                    .fill(readinessAccentColor)
                    .frame(width: 6, height: 6)
                Text(recipeEyebrowText)
                    .font(DS.Font.mono(10.5, weight: .medium))
                    .kerning(0.9)
                    .textCase(.uppercase)
    ...
    private var recipeEyebrowText: String {
        if isReadyToCook {
    ...
    private var readinessAccentColor: Color {
        ...
        case .readyToCook, .coveredByShoppingList:
    ...
    private func recipeMetaChip(icon: String, text: String) -> some View {
        HStack(spacing: 5) {
    ...
    private func dietaryTagsSection(tags: [RecipeDietaryTag]) -> some View {
        VStack(alignment: .leading, spacing: DS.Spacing.xs) {
            RecipeDetailSectionLabel(viewModel.localizer.text(.recipeDietaryTags))
    ...
    private func toggleFollowAuthor() {
        guard let recipeCreatorID = validRecipeCreatorID, canShowFollowButton else {
            ...
            SeasonLog.debug("[SEASON_FOLLOW_IDENTITY] phase=button_tap_blocked_invalid_creator recipe_id=\(rankedRecipe.recipe.id) raw_creator_id=\(rawCreatorForLog) canonical_creator_id=\(validRecipeCreatorID ?? "nil")")
            return
        }
        let wasFollowing = followStore.isFollowing(recipeCreatorID)
        SeasonLog.debug("[SEASON_FOLLOW_IDENTITY] phase=recipe_toggle recipe_id=\(rankedRecipe.recipe.id) creator_id=\(recipeCreatorID) creator_name=\(displayedCreatorName) was_following=\(wasFollowing)")
    ...
    private func logCreatorEvaluation() {
        ...
        SeasonLog.debug("[SEASON_FOLLOW_IDENTITY] phase=identity_eval recipe_id=\(rankedRecipe.recipe.id) raw_creator_id=\(rawCreatorForLog) canonical_creator_id=\(canonicalCreatorForLog) displayed_creator_name=\(creatorDisplay) current_creator_id=\(currentID) can_show_follow=\(canShow)")
        SeasonLog.debug("[SEASON_FOLLOW_SYNC] phase=creator_sync_eligibility recipe_id=\(rankedRecipe.recipe.id) creator_id=\(canonicalCreatorForLog) eligible=\(backendSyncEligible)")
        if let validRecipeCreatorID {
            SeasonLog.debug("[SEASON_FOLLOW_IDENTITY] phase=canonical_id_resolved recipe_id=\(rankedRecipe.recipe.id) creator_id=\(validRecipeCreatorID)")
            if canShow {
                SeasonLog.debug("[SEASON_RECIPE_CREATOR] phase=follow_visible recipe_id=\(rankedRecipe.recipe.id) creator_id=\(validRecipeCreatorID)")
            }
        } else if !rawCreatorID.isEmpty {
            SeasonLog.debug("[SEASON_FOLLOW_IDENTITY] phase=invalid_follow_identifier value=\(rawCreatorID)")
            if rawCreatorID.range(of: "^[A-Za-z0-9_\\-.]+$", options: .regularExpression) != nil &&
                !rawCreatorID.contains("-") {
                SeasonLog.debug("[SEASON_FOLLOW_IDENTITY] phase=legacy_name_rejected value=\(rawCreatorID)")
            }
            SeasonLog.debug("[SEASON_RECIPE_CREATOR] phase=follow_hidden_invalid_creator recipe_id=\(rankedRecipe.recipe.id) raw_creator_id=\(rawCreatorForLog)")
        }
        ...
        case .legacyUnmigrated(let rawLegacyID):
            SeasonLog.debug("[SEASON_RECIPE_CREATOR] phase=legacy_recipe_identity_detected recipe_id=\(rankedRecipe.recipe.id) raw_creator_id=\(rawLegacyID) display_name=\(displayedCreatorName)")
        case .unknown:
            if !rankedRecipe.recipe.hasDisplayableCreatorIdentity {
                SeasonLog.debug("[SEASON_RECIPE_CREATOR] phase=follow_hidden_invalid_creator recipe_id=\(rankedRecipe.recipe.id) raw_creator_id=nil")
            }
        }
    }
    ...
    private func logDetailIdentity() {
        ...
        SeasonLog.debug("[SEASON_CREATOR_CHAIN] phase=detail_identity recipe_id=\(rankedRecipe.recipe.id) creator_id=\(rawCreatorForLog) canonical_creator_id=\(canonicalCreatorIDForLog) creator_display_name=\(creatorDisplayForLog) author=\(rankedRecipe.recipe.author) current_creator_id=\(currentID) can_show_follow=\(canShow)")

        if showsUserAuthorshipMetadata && rankedRecipe.recipe.canonicalCreatorID == nil {
            SeasonLog.debug("[SEASON_CREATOR_CHAIN] phase=missing_canonical_creator_id_in_detail recipe_id=\(rankedRecipe.recipe.id) title=\(rankedRecipe.recipe.title) creator_display_name=\(creatorDisplayForLog) author=\(rankedRecipe.recipe.author)")
        }
    }
    ...
    private func performRuntimeTranslation(using session: TranslationSession) async {
        ...
        do {
            ...
            SeasonLog.debug("[SEASON_TRANSLATION] phase=failed error=\(error)")
        }
    }
    ...
    private var ingredientSummaryRow: some View {
        HStack(spacing: 8) {
            Text(
                ...
                .foregroundStyle(SeasonChipSemantic.warning.foreground)
                .padding(.horizontal, 9)
                .padding(.vertical, 5)
                .background(
                    Capsule(style: .continuous)
                        .fill(SeasonChipSemantic.warning.background)
                )
            ...
            Text(
                ...
                .font(DS.Font.sans(12, weight: .semibold))
                .foregroundStyle(SeasonChipSemantic.positive.foreground)
                .padding(.horizontal, 9)
                .padding(.vertical, 5)
                .background(
                    Capsule(style: .continuous)
                        .fill(SeasonChipSemantic.positive.background)
                )
        ...
        if !rankedRecipe.recipe.externalMedia.isEmpty || !recipeSocialLinks.isEmpty || originalSourceURL != nil {
            VStack(alignment: .leading, spacing: 10) {
                if !rankedRecipe.recipe.externalMedia.isEmpty {
                    RecipeDetailSectionLabel(viewModel.localizer.text(.watchVideo))
...
private struct RecipeDetailSectionLabel: View {
    let text: String
...
private struct RecipeHeroView: View {
            ...
            .overlay(alignment: .bottomLeading) {
                LinearGradient(
                    stops: [
                        ...
                        .init(color: Color.black.opacity(0.22), location: 0.28),
    ...
    private var fallbackImageView: some View {
        ZStack {
            LinearGradient(
                colors: [
                    DS.Color.sageSoft.opacity(0.92),
                    DS.Color.bgSub,
                    DS.Color.cardSoft
                ],
            ...
            Image(systemName: "fork.knife.circle.fill")
                ...
                .foregroundColor(DS.Color.sageDeep.opacity(0.48))
        }
    }
            ...
            if canShowFollowButton {
                Button(action: onFollowTap) {
                    Text(isFollowing ? followingLabel : followLabel)
                        .font(DS.Font.sans(12, weight: .semibold))
                        .tracking(0.3)
                        .foregroundStyle(isFollowing ? DS.Color.card : DS.Color.ink)
                        ...
                        .overlay(
                            Capsule(style: .continuous)
                                .stroke(DS.Color.borderS, lineWidth: 1)
                        )
                }
                .buttonStyle(PressableCardButtonStyle())
            }
...
private struct RecipeIntelligenceCard: View {
    ...
    @ViewBuilder
    private var intelligenceColumns: some View {
        IntelligenceColumn(
            label: fridgeLabel,
            value: fridgeMatchText,
            detail: fridgeDetailText,
            tint: DS.Color.sageDeep
        )
        ...
        IntelligenceColumn(
            ...
            tint: isReadyToCook ? DS.Color.fresh : DS.Color.ochre
        )
    }
}
...
private struct IntelligenceDivider: View {
    var body: some View {
        Rectangle()
            ...
            HStack(spacing: 12) {
                if let icon {
                    Image(systemName: icon)
                        ...
                        .background(
                            Circle()
                                .fill(Color.white.opacity(style == .primary ? 0.16 : 0.7))
                        )
                }
...
private struct IngredientRowView: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            if let item = ingredient.item {
                ProduceThumbnailView(item: item, size: 38)
            } else {
                Image(systemName: "leaf")
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: DS.Radius.sm, style: .continuous)
                            .fill(DS.Color.sageSoft.opacity(0.52))
                    )
            }

            VStack(alignment: .leading, spacing: 2) {
                Text(displayName)
                    .font(DS.Font.sans(13.5, weight: .medium))
                    .foregroundStyle(DS.Color.ink)
...
private struct RecipeStepView: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 14) {
            Text("\(index + 1)")
                .font(DS.Font.serif(14, weight: .medium))
    ...
    var body: some View {
        if !reasons.isEmpty {
            VStack(alignment: .leading, spacing: 9) {
                RecipeDetailSectionLabel(title)
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: 6) {
                        ForEach(Array(reasons.enumerated()), id: \.offset) { _, reason in
                            ReasonChip(kind: .fridge, text: reason)
                        }
                    }
                }
            }
